...
const App = (() => {
  ...
  // Floating shortcut to the AI assistant. Mounted here rather than in
  // each page's markup so every screen that renders the app shell gets
  // it — skipped on ask.html itself, where it would just link to the
  // page you're already on. Auth screens don't mount the shell at all,
  // so they're excluded automatically.
  function mountAIFab() {
    const current = window.location.pathname.split('/').pop() || 'index.html';
    if (current === 'ask.html' || current.startsWith('_demo-ask')) return;
    ...
    fab.href = 'ask.html';
    fab.setAttribute('aria-label', 'Ask Momsly');
