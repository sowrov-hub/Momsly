...
const Tracker = (() => {
  ...
  // Breastfeed and Pump live as sub-options under the single "Feeding"
  // tab instead of separate top-level tabs, to keep the main horizontal
  // row short. Each remains its own distinct log type underneath.
  const FEEDING_GROUP = ['feeding', 'breastfeeding', 'pump'];
  const FEEDING_SUBTAB_LABELS = { feeding: 'Bottle', breastfeeding: 'Breastfeed', pump: 'Pump' };
  ...
  function renderTabs() {
    ...
    const mainTypes = Data.TRACKER_TYPES.filter(t => !FEEDING_GROUP.includes(t.id) || t.id === 'feeding');

    wrap.innerHTML = mainTypes.map(t => {
      const isActive = t.id === 'feeding' ? FEEDING_GROUP.includes(activeType) : t.id === activeType;
      return `
      <button class="tracker-tab ${isActive ? 'active' : ''}" data-type="${t.id}">
        <span class="tracker-tab__icon">${(isActive ? IconsFilled : Icons)[t.icon]}</span>
    ...
    wrap.querySelectorAll('.tracker-tab').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (type === 'feeding') {
          setActiveType(FEEDING_GROUP.includes(activeType) ? activeType : 'feeding');
          return;
        }
    ...
    renderFeedingSubtabs();
  }

  function renderFeedingSubtabs() {
    const wrap = document.getElementById('tracker-subtabs');
    if (!wrap) return;
    if (!FEEDING_GROUP.includes(activeType)) { wrap.innerHTML = ''; wrap.style.display = 'none'; return; }

    wrap.style.display = 'flex';
    const premiumActive = Auth.isPremiumActive();
    wrap.innerHTML = FEEDING_GROUP.map(id => {
      const def = Data.trackerType(id);
      const isActive = id === activeType;
      return `<button class="tracker-subtab ${isActive ? 'active' : ''}" data-subtype="${id}">
        <span class="tracker-subtab__icon">${(isActive ? IconsFilled : Icons)[def.icon]}</span>
        <span>${FEEDING_SUBTAB_LABELS[id]}${def.premium && !premiumActive ? ' 🔒' : ''}</span>
      </button>`;
    }).join('');

    wrap.querySelectorAll('.tracker-subtab').forEach(btn => {
      btn.addEventListener('click', () => {
        const type = btn.dataset.subtype;
        const def = Data.trackerType(type);
        if (def.premium && !Auth.isPremiumActive()) { window.location.href = 'upgrade.html'; return; }
        setActiveType(type);
      });
  ...
  function renderTimerCard() {
    ...
    if (!wrap) return;
    if (FEEDING_GROUP.includes(activeType)) { wrap.innerHTML = feedingTimerHTML(); bindFeedingTimer(); }
  ...
  const FEEDING_TIMER_LABELS = {
    feeding: { title: 'Smart Feeding Timer', start: 'Start Feeding', save: 'Save Feed' },
    breastfeeding: { title: 'Smart Breastfeeding Timer', start: 'Start Breastfeeding', save: 'Save Session' },
    pump: { title: 'Smart Pump Timer', start: 'Start Pumping', save: 'Save Session' },
  };

  function feedingTimerHTML() {
    const labels = FEEDING_TIMER_LABELS[activeType] || FEEDING_TIMER_LABELS.feeding;
    const premiumActive = Auth.isPremiumActive();
    if (!premiumActive) {
      return `<div class="card tool-card--locked" style="text-align:center;"><span class="badge badge--premium">${Icons.crown}Premium</span><h3 style="margin-top:8px;">${labels.title}</h3><p class="subtext">Live timer with next-feed prediction.</p><a href="upgrade.html" class="btn btn--primary" style="margin-top:12px;">Unlock Premium</a></div>`;
    }
    const active = Feeding.getActive();
    const child = activeChild();
    const nextFeed = child ? Feeding.nextFeedPrediction(child.id) : null;
    // Only show the "next feed" prediction while a timer for THIS type
    // is what's running (or none is), so switching sub-tabs doesn't show
    // a stale prediction next to the wrong timer.
    const showPrediction = nextFeed && (!active || active.type === activeType);
    return `
      <div class="card timer-card ${active && active.type === activeType ? 'timer-active' : ''}">
        <div class="eyebrow" style="color:rgba(255,255,255,.8)">${labels.title}</div>
        <div class="timer-display" id="feed-timer-display">${active && active.type === activeType ? Utils.formatTimer(Feeding.elapsedMs()) : '00:00'}</div>
        ${showPrediction ? `<p style="opacity:.9;font-size:13px;">Next feed around ${Utils.formatTime(nextFeed)}</p>` : ''}
        <div class="timer-controls">
          ${active && active.type === activeType
            ? `<button class="btn btn--secondary" id="feed-cancel">${Icons.close} Cancel</button><button class="btn" style="background:#fff;color:var(--color-primary);" id="feed-stop">${Icons.stop} ${labels.save}</button>`
            : active
              ? `<p style="font-size:13px;opacity:.9;">A ${FEEDING_TIMER_LABELS[active.type]?.title.replace('Smart ', '').replace(' Timer', '') || 'feeding'} timer is already running.</p>`
              : `<button class="btn" style="background:#fff;color:var(--color-primary);" id="feed-start">${Icons.play} ${labels.start}</button>`}
        </div>
      </div>`;
  }
  ...
  function bindFeedingTimer() {
    ...
    document.getElementById('feed-start')?.addEventListener('click', () => {
      ...
      Feeding.start(child.id, activeType);
      renderTimerCard();
    });
    ...
    document.getElementById('feed-stop')?.addEventListener('click', async () => {
      if (activeType === 'breastfeeding') {
        const side = prompt('Which side? (left / right / both)', 'left');
        Feeding.stop({ side: side || null });
      } else {
        const amount = prompt('Amount (oz)? Leave blank to skip.');
    ...
    if (Feeding.getActive() && Feeding.getActive().type === activeType) {
  ...
  function renderHistory() {
    ...
    // Editing covers deleting too — the edit sheet carries its own
    // Delete Entry action, so there's one place to correct or remove.
    wrap.querySelectorAll('[data-edit-log]').forEach(btn => {
      btn.addEventListener('click', () => LogEditor.open(btn.dataset.editLog, renderTypeScreen));
    });
  }
  ...
  function renderChart() {
    ...
    const detailEl = document.getElementById('chart-point-detail');
    ...
    if (['weight', 'height', 'temperature'].includes(activeType)) {
      ...
      Charts.attachPointClickHandler(canvas, (pt) => {
        const detailEl = document.getElementById('chart-point-detail');
        if (!detailEl) return;
        detailEl.textContent = `${pt.label}: ${pt.value} ${typeDef.unit || ''}`;
