  ...
  function toolCard(tool, { isFavorite = false, isLocked = false } = {}) {
    return `
      <a class="tool-card card--interactive ${isLocked ? 'tool-card--locked' : ''}" href="${isLocked ? 'upgrade.html' : tool.route}" data-tool-id="${tool.id}">
        ...
        <h3>${tool.name}</h3>
        <p>${tool.desc}</p>
        ${tool.premium ? `<span class="badge badge--premium">${Icons.crown}Premium</span>` : ''}
      </a>`;
  }
  ...
  function logItem(log) {
    ...
    return `
      <div class="log-item" data-log-id="${log.id}">
        ...
        <button class="icon-btn" data-delete-log="${log.id}" aria-label="Delete entry">${Icons.trash}</button>
      </div>`;
  }
    ...
    return `
      <div class="log-item" data-reminder-id="${reminder.id}">
        ...
        <div class="log-info">
          ...
          <div class="log-meta">${reminder.repeat === 'interval' ? `Every ${reminder.every} min` : Utils.formatDateTime(reminder.time)}</div>
        </div>
        <label class="switch">
