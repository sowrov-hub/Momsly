...
const Auth = (() => {
  ...
  async function loadOrCreateProfile(supaUser) {
    ...
    if (existing) return backfillMissingTrialStart(client, await reconcilePremiumDrift(client, existing));
    ...
    if (insertError) {
      ...
      if (refetched) return backfillMissingTrialStart(client, await reconcilePremiumDrift(client, refetched));
  ...
  // Fixes a real bug: if trial_started_at is ever missing/null on an
  // EXISTING profile row (a row that already exists, so this is not a
  // brand-new signup), trialInfo() below falls back to showing the full
  // 7 days — and since nothing ever wrote a value into that null field,
  // every future page load hit the exact same fallback forever. Someone
  // could stay stuck showing "7 days left" indefinitely no matter how
  // much time actually passed. This backfills it once, the first time
  // it's noticed, and persists that so the countdown becomes stable
  // from that point forward. It can't retroactively know the account's
  // true original signup date if that was never recorded — see the
  // SQL note for manually correcting a specific account's date.
  async function backfillMissingTrialStart(client, profile) {
    if (!profile || profile.trial_started_at) return profile;
    const now = new Date().toISOString();
    const { error } = await client.from('profiles').update({ trial_started_at: now }).eq('id', profile.id);
    if (error) { console.warn('Backfilling trial_started_at failed', error); return profile; }
    console.warn('profiles row for', profile.id, 'had no trial_started_at — backfilled to now.');
  ...
  async function signup({ name, email, password }) {
    ...
    if (typeof Scheduler !== 'undefined' && typeof Scheduler.start === 'function') Scheduler.start();
  ...
  async function init() {
    ...
    initPromise = (async () => {
      ...
      if (cachedUser && typeof Scheduler !== 'undefined' && typeof Scheduler.start === 'function') Scheduler.start();
    })();
    return initPromise;
  }
