...
const Auth = (() => {
  ...
  // Where a locked-out account is sent. Once the free trial ends the
  // WHOLE app is locked — not just the premium tools — so every
  // protected page bounces here until Lifetime Premium is unlocked.
  ...
  // Is this account shut out of the app entirely?
  //
  // True only for a logged-in, non-premium user whose 3-day trial has
  // run out. Signed-out visitors are NOT "locked" — they're handled by
  // the welcome/login redirect instead, which is a different journey.
  function isLocked() {
    return !!cachedUser && !isPremiumActive();
  }
  ...
  // Unlocks Lifetime Premium for this account.
  //
  // The access code is checked in the page (upgrade.html) and the flag is
  // written straight from the browser, which is the simple setup Momsly
  // has always used — no Edge Function, nothing extra to deploy.
  ...
  async function init() {
    ...
    initPromise = (async () => {
      ...
      // Trial over, no purchase → lock the entire app.
      //
      // Previously only tools flagged `premium: true` were gated and
      // everything else stayed usable forever. Now the trial gates the
      // whole product: every protected page redirects to the upgrade
      // screen. PUBLIC_PAGES stay reachable so a locked user can still
      // read the terms, log out, or sign in as someone else.
