...
const Charts = (() => {
  ...
  // Renders bars with the most recent (last) value highlighted in the
  // full brand gradient — others in a paler tint — plus a dashed average
  // line and a value callout above the highlighted bar. Mirrors the
  // "today vs. average" bar treatment from the design reference.
  function barChart(canvas, labels, values, { unit = '' } = {}) {
    ...
    const padTop = 30, padBottom = 24, padSide = 8;
    const chartH = height - padTop - padBottom;
    const max = Math.max(...values, 1);
    const barW = (width - padSide * 2) / values.length * 0.5;
    ...
    if (avg > 0) {
      ...
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(padSide, avgY);
      ctx.lineTo(width - padSide, avgY);
      ctx.stroke();
      ctx.restore();
    }
    ...
    values.forEach((v, i) => {
      ...
      const barH = Math.max((v / max) * chartH, v > 0 ? 4 : 2);
      const y = padTop + (chartH - barH);
      const isHighlight = i === highlightIndex && v > 0;
      ...
      if (v > 0) {
        ...
        if (isHighlight) {
          ...
          grad.addColorStop(1, colors.secondary + '40');
        }
        ctx.fillStyle = grad;
      } else {
      ...
      if (isHighlight) {
        ...
        ctx.fillText(v + (unit ? ' ' + unit : ''), x + barW / 2, y - 14);

        ctx.beginPath();
        ctx.arc(x + barW / 2, y, 4.5, 0, Math.PI * 2);
        ctx.fillStyle = '#fff';
        ctx.fill();
        ctx.lineWidth = 2.5;
