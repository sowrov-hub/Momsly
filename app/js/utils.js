...
const Utils = (() => {
  ...
  // Calendar-day key in the user's OWN timezone. This deliberately does
  // not use toISOString(), which yields the UTC date: for anyone ahead of
  // UTC, everything logged between local midnight and the UTC rollover
  // was being filed under the previous day — so "Feeds today" read 0
  // through the early hours, and those entries landed on the wrong
  // calendar square. Every "is this the same day" check in the app goes
  // through here, so local-time is the correct basis for all of them.
