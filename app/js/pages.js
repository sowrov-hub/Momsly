...
const HomePage = (() => {
  ...
  function renderUpcomingReminders() {
    ...
    const upcoming = Scheduler.list()
      .filter(r => r.enabled && !(r.repeat === 'once' && r.lastFiredAt))
      .map(r => ({ r, next: Scheduler.nextFireTime(r, now) }))
      .sort((a, b) => a.next - b.next)
      .slice(0, 4)
      .map(({ r }) => r);

    el.innerHTML = upcoming.length
      ? upcoming.map(r => summaryCard({
          icon: r.icon, title: r.title,
          meta: scheduleLabel(r),
          value: '', kind: 'reminder', id: r.id,
        })).join('')
      : Components.emptyState({ icon: 'bell', title: 'No upcoming reminders', subtitle: 'Add one from Tools to get notified automatically.' });

    el.querySelectorAll('[data-summary-card]').forEach(card => {
      card.addEventListener('click', () => openReminderDetail(card.dataset.id));
    });
  }
