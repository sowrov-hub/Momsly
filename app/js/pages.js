...
const HomePage = (() => {
      ...
      statsCard.innerHTML = `
        <div class="hero-stat-item hero-stat-item--feeds">
          <div class="hero-stat-item__icon-wrap">
            <span class="hero-stat-item__icon"><img src="assets/images/icon-feeds.png" alt=""></span>
          </div>
        ...
        <div class="hero-stat-item hero-stat-item--naps">
          <div class="hero-stat-item__icon-wrap">
            <span class="hero-stat-item__icon"><img src="assets/images/icon-naps.png" alt=""></span>
          </div>
        ...
        <div class="hero-stat-item hero-stat-item--sleep">
          <div class="hero-stat-item__icon-wrap">
            <span class="hero-stat-item__icon"><img src="assets/images/icon-sleep.png" alt=""></span>
          </div>
  ...
  function renderTodaySummary() {
    ...
    // Baby expenses — total spent so far, tap through for the full list.
    const budget = Storage.get('budget') || { entries: [] };
    if (budget.entries.length) {
      const total = budget.entries.reduce((s, e) => s + e.amount, 0);
      cards.push(summaryCard({
        icon: 'wallet', title: 'Baby Expenses',
        meta: `${budget.entries.length} ${budget.entries.length === 1 ? 'expense' : 'expenses'} logged`,
    ...
    el.querySelectorAll('[data-summary-card]').forEach(card => {
      card.addEventListener('click', () => {
        ...
        else if (kind === 'expenses') openExpensesDetail();
  ...
  function openExpensesDetail() {
    ...
    title.textContent = 'Baby Expenses';
    body.innerHTML = `
      <div class="card" style="text-align:center;background:var(--gradient-soft);margin-bottom:var(--space-md);">
      ...
      <div style="max-height:280px;overflow-y:auto;margin-bottom:var(--space-md);">
        ${budget.entries.slice().reverse().map(e => `
          <div class="log-item">
  ...
  function renderGrowthTrends() {
    ...
    if (!section) return;
    const child = activeChild();
    if (!child) { section.style.display = 'none'; return; }

    const allLogs = Storage.get('logs') || [];
    const weightLogs = allLogs.filter(l => l.type === 'weight' && l.childId === child.id).sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
    const heightLogs = allLogs.filter(l => l.type === 'height' && l.childId === child.id).sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));

    if (weightLogs.length < 2 && heightLogs.length < 2) { section.style.display = 'none'; return; }
    ...
    } else if (weightLatestEl) {
      weightLatestEl.innerHTML = `<span style="font-size:11px;color:var(--color-subtext);">Log 2+ entries to see a trend</span>`;
    }
    ...
    } else if (heightLatestEl) {
      heightLatestEl.innerHTML = `<span style="font-size:11px;color:var(--color-subtext);">Log 2+ entries to see a trend</span>`;
    }
  }
  ...
  function renderRecentTools() {
    ...
    // Favorited tools always show first, but they top up to 4 with the
    // default catalog order rather than replacing it — favoriting one
    // tool used to wipe out the rest of the Featured grid entirely.
    const favTools = Data.TOOLS.filter(t => favs.includes(t.id));
    const fillerTools = Data.TOOLS.filter(t => !favs.includes(t.id)).slice(0, Math.max(0, 4 - favTools.length));
    const featured = [...favTools, ...fillerTools].slice(0, 4);
