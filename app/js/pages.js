...
const HomePage = (() => {
  ...
  function renderHero() {
    ...
    const stage = document.getElementById('hero-illustration-pills');
    if (stage) {
      stage.innerHTML = `
        <div class="floating-pill">
          <span class="floating-pill__icon">${Icons.bottle}</span>
          <div><b>${feedSummary.count}</b><span>Feeds today</span></div>
        </div>
        <div class="floating-pill">
          <span class="floating-pill__icon">${Icons.sleep}</span>
          <div><b>${sleepSummary.count}</b><span>Naps today</span></div>
        </div>
        <div class="floating-pill">
          <span class="floating-pill__icon">${Icons.moon}</span>
          <div><b>${Utils.durationLabel(sleepSummary.totalMs)}</b><span>Sleep today</span></div>
        </div>`;
    }
  }
  ...
  function renderTodaySummary() {
    ...
    el.querySelectorAll('[data-summary-card]').forEach(card => {
      card.addEventListener('click', () => {
        if (card.dataset.kind === 'reminder') openReminderDetail(card.dataset.id);
