...
const Tools = (() => {
  ...
  // 'once' needs a real calendar date (a doctor's appointment is on a day,
  // not just at a time); 'daily' only needs the time of day; 'interval'
  // needs neither, just the gap between repeats.
  function toggleReminderFields() {
    ...
    if (dateField) dateField.style.display = repeat === 'once' ? 'block' : 'none';
  ...
  async function handleAddReminder(e) {
    ...
    if (repeat !== 'interval' && timeVal) {
      ...
      let d;
      if (repeat === 'once' && dateVal) {
        // A specific day was chosen — honour it exactly, including dates
        // months out, rather than assuming today.
        const [y, mo, day] = dateVal.split('-').map(Number);
        d = new Date(y, mo - 1, day, h, m, 0, 0);
      } else {
        d = new Date();
        d.setHours(h, m, 0, 0);
        // No date given on a one-off: keep the old behaviour of meaning
        // "today, or tomorrow if that time has already gone".
  ...
  // Blank the form every time the sheet is opened.
  ...
  return {
    ...
    bindReminderListEvents, bindReminderSheetReset, resetReminderForm,
