...
const Tools = (() => {
  ...
  function renderReminderPresets() {
    ...
    wrap.querySelectorAll('.chip').forEach(chip => {
      chip.addEventListener('click', () => {
        ...
        if (preset.every) {
          document.getElementById('reminder-repeat').value = 'interval';
          document.getElementById('reminder-every').value = preset.every;
  ...
  function bindReminderListEvents() {
    ...
    list.addEventListener('click', (e) => {
      ...
      if (del) { Scheduler.remove(del.dataset.deleteReminder); renderReminders(); UI.toast('Reminder deleted.'); }
    });
  ...
  /* ---------------- Checklists (hospital bag / school / packing) ---------------- */
  const CHECKLIST_DEFAULTS = {
    hospitalBag: ['Going-home outfit', 'Diapers & wipes', 'Phone charger', 'Nursing bra / pump', 'Snacks for labor', 'Insurance card & ID', 'Car seat installed'],
    school: ['Backpack labeled', 'School supplies', 'Lunch box', 'Emergency contact form', 'Immunization records', 'Weather-appropriate clothes'],
    packing: ['Diapers & wipes', 'Stroller', 'Car seat', 'Medicine & first aid', "Baby's favorite toy", 'Bottles & formula', 'Sleep sack'],
    shopping: ['Diapers', 'Wipes', 'Formula / baby food'],
  };

  function getChecklist(key) {
    const all = Storage.get('checklists') || {};
    if (!all[key]) {
      all[key] = (CHECKLIST_DEFAULTS[key] || []).map(text => ({ id: Utils.uid(), text, done: false }));
      Storage.set('checklists', all);
    }
    return all[key];
  }
  ...
  function renderChecklist(key, containerId) {
    ...
    el.innerHTML = items.map(i => `
      <label class="checkbox-row" style="padding:10px 0;border-bottom:1px solid var(--color-border);">
        <input type="checkbox" data-checklist="${key}" data-item="${i.id}" ${i.done ? 'checked' : ''} />
        <span style="${i.done ? 'text-decoration:line-through;color:var(--color-subtext);' : ''}">${Utils.escapeHtml(i.text)}</span>
      </label>`).join('');
  }

  function bindChecklistEvents(key, containerId) {
    const el = document.getElementById(containerId);
    if (!el) return;
    el.addEventListener('change', (e) => {
      const cb = e.target.closest('[data-checklist]');
      if (!cb) return;
      const all = Storage.get('checklists') || {};
      const list = all[cb.dataset.checklist];
      const item = list.find(i => i.id === cb.dataset.item);
      if (item) { item.done = cb.checked; Storage.set('checklists', all); renderChecklist(key, containerId); }
    });
  }

  function addChecklistItem(key, containerId, text) {
    if (!text.trim()) return;
    const all = Storage.get('checklists') || {};
    all[key] = all[key] || [];
    all[key].push({ id: Utils.uid(), text: text.trim(), done: false });
    Storage.set('checklists', all);
  ...
  function addMoodEntry(mood, note) {
    Storage.pushItem('moodLog', { id: Utils.uid(), date: Utils.nowISO(), mood, note: note || '' });
  }

  function renderMoodLog(containerId) {
    const el = document.getElementById(containerId);
    if (!el) return;
    const entries = (Storage.get('moodLog') || []).slice().reverse().slice(0, 20);
    el.innerHTML = entries.length ? entries.map(e => `
      <div class="log-item">
        <div class="log-icon">${Icons.smile}</div>
        <div class="log-info">
          <div class="log-title">${Utils.escapeHtml(e.mood)}</div>
          <div class="log-meta">${Utils.formatDateTime(e.date)}${e.note ? ' · ' + Utils.escapeHtml(e.note) : ''}</div>
        </div>
      </div>`).join('') : Components.emptyState({ icon: 'journal', title: 'No entries yet', subtitle: 'Check in with yourself whenever you need to.' });
  }
  ...
  function setSavingsGoal(name, target) {
    const existing = Storage.get('savingsGoal') || { saved: 0 };
    Storage.set('savingsGoal', { name, target: Number(target), saved: existing.saved || 0 });
  }

  function addSavings(amount) {
    const goal = Storage.get('savingsGoal');
    if (!goal) return;
    goal.saved = (goal.saved || 0) + Number(amount);
    Storage.set('savingsGoal', goal);
  }
  ...
  return {
    ...
    renderReminders, renderReminderPresets, toggleReminderFields, handleAddReminder, bindReminderListEvents,
    getChecklist, renderChecklist, bindChecklistEvents, addChecklistItem,
    addMoodEntry, renderMoodLog,
    addExpense, renderBudget, setSavingsGoal, addSavings,
